...
void fill_flash_buffer(RDRAM_ARG PTR(void) dramAddr, uint32_t count, char value) {
    for (uint32_t i = 0; i < count; i++) {
        MEM_B(i, dramAddr) = value;
    }
}
...
extern "C" void osFlashReadArray_recomp(uint8_t * rdram, recomp_context * ctx) {
    ...
    if (page_num >= page_count || n_pages > page_count - page_num) {
        uint32_t count = n_pages <= page_count ? n_pages * page_size : page_size;
        fill_flash_buffer(PASS_RDRAM dramAddr, count, static_cast<char>(0xFF));
        osSendMesg(PASS_RDRAM mq, 0, OS_MESG_NOBLOCK);
        ctx->r2 = -1;
        return;
    }
