...
extern "C" void load_overlays(uint32_t rom, int32_t ram_addr, uint32_t size) {
    const uint64_t load_start = rom;
    const uint64_t load_end = load_start + size;

    for (size_t i = 0; i < sections_info.num_code_sections; i++) {
        const SectionTableEntry& section = sections_info.code_sections[i];

        if (section.rom_addr == UINT32_MAX) {
            break;
        }

        const uint64_t section_start = section.rom_addr;
        const uint64_t section_end = section_start + section.size;

        if (section_start >= load_end) {
            break;
        }

        if (section_end > load_start) {
            load_overlay(i, static_cast<int32_t>(section_start - load_start + static_cast<uint32_t>(ram_addr)));
        }
    }
}
