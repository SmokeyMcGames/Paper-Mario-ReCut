...
void read_save_file() {
    ...
    if (recomp::get_save_type() == recomp::SaveType::Flashram || recomp::get_save_type() == recomp::SaveType::AllowAll) {
        default_value = static_cast<char>(0xFF);
    }
    std::fill(save_context.save_buffer.begin(), save_context.save_buffer.end(), default_value);
