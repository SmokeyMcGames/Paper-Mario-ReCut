...
namespace RT64 {
...
#ifdef RT64_PAPER_MARIO_TRANSFORM_MATCHING
    uint32_t paperMarioTransformId(uint32_t category, uint32_t physicalAddress) {
        uint32_t id = 0x504D0000U ^ (category << 24) ^ (physicalAddress >> 2);
        id ^= id >> 16;
        id *= 0x7FEB352DU;
        id ^= id >> 15;
        id *= 0x846CA68BU;
    ...
    uint32_t paperMarioAppendMatrixGroup(DrawData &drawData, uint32_t baseGroupIndex, uint32_t physicalAddress, uint32_t category, bool projection) {
        if (physicalAddress == 0) {
            return baseGroupIndex;
        }

        const TransformGroup &baseGroup = drawData.transformGroups[baseGroupIndex];
        if (baseGroup.matrixId != G_EX_ID_AUTO) {
            return baseGroupIndex;
        }

        TransformGroup paperMarioGroup = baseGroup;
        paperMarioGroup.matrixId = paperMarioTransformId(category, physicalAddress);
        paperMarioGroup.decompose = !projection;
        paperMarioGroup.positionInterpolation = G_EX_COMPONENT_INTERPOLATE;
        paperMarioGroup.rotationInterpolation = G_EX_COMPONENT_INTERPOLATE;
        paperMarioGroup.scaleInterpolation = G_EX_COMPONENT_INTERPOLATE;
        paperMarioGroup.skewInterpolation = G_EX_COMPONENT_INTERPOLATE;
        paperMarioGroup.perspectiveInterpolation = G_EX_COMPONENT_INTERPOLATE;
        paperMarioGroup.vertexInterpolation = G_EX_COMPONENT_SKIP;
        paperMarioGroup.tileInterpolation = G_EX_COMPONENT_SKIP;
        paperMarioGroup.ordering = G_EX_ORDER_LINEAR;

        const uint32_t groupIndex = uint32_t(drawData.transformGroups.size());
        drawData.transformGroups.emplace_back(paperMarioGroup);
        return groupIndex;
    }
#endif
...
#   endif
    ...
    void RSP::addCurrentProjection(Projection::Type type) {
        ...
        if (projectionMatrixChanged || viewportChanged) {
            ...
            uint32_t viewProjGroupIndex = extended.curViewProjMatrixIdGroupIndex;
#ifdef RT64_PAPER_MARIO_TRANSFORM_MATCHING
            viewProjGroupIndex = paperMarioAppendMatrixGroup(drawData, viewProjGroupIndex, physicalAddress, 0x01U, true);
#endif
            ...
            drawData.viewProjTransformGroups.emplace_back(viewProjGroupIndex);
    ...
    void RSP::setVertexCommon(uint8_t dstIndex, uint8_t dstMax) {
        ...
        if (extended.modelMatrixIdStackChanged) {
            ...
            extended.curModelMatrixIdGroupIndex = int(drawData.transformGroups.size());
            drawData.transformGroups.emplace_back(extended.modelMatrixIdStack[stackIndex]);
            extended.modelMatrixIdStackChanged = false;
        }
        
        // ModelViewProj is only updated when a vertex is processed and the flag is enabled.
        auto &worldTransforms = drawData.worldTransforms;
        auto &worldTransformGroups = drawData.worldTransformGroups;
        auto &worldTransformSegmentedAddresses = drawData.worldTransformSegmentedAddresses;
        auto &worldTransformPhysicalAddresses = drawData.worldTransformPhysicalAddresses;
        auto &worldTransformVertexIndices = drawData.worldTransformVertexIndices;
...
#       endif
        ...
        if (addWorldTransform) {
            ...
            uint32_t worldGroupIndex = extended.curModelMatrixIdGroupIndex;
#ifdef RT64_PAPER_MARIO_TRANSFORM_MATCHING
            worldGroupIndex = paperMarioAppendMatrixGroup(drawData, worldGroupIndex, physicalAddress, 0x02U, false);
#endif
            workload.physicalAddressTransformMap.emplace(physicalAddress, uint32_t(worldTransformGroups.size()));
            worldTransformGroups.emplace_back(worldGroupIndex);
