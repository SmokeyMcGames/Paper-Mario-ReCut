...
namespace RT64 {
    ...
    struct SharedQueueResources {
        ...
        void setUserConfig(const UserConfiguration &newUserConfig, bool discardFBs) {
            bool interpolationConfigChanged = false;
            {
                std::scoped_lock<std::mutex> configurationLock(configurationMutex);
                interpolationConfigChanged =
                    (userConfig.refreshRate != newUserConfig.refreshRate) ||
                    (userConfig.refreshRateTarget != newUserConfig.refreshRateTarget);
            ...
            if (interpolationConfigChanged) {
                std::scoped_lock<std::mutex> interpolatedLock(interpolatedMutex);
                for (InterpolatedFrameCounters &frameCounters : interpolatedFrames) {
                    frameCounters.presented = frameCounters.count;
                    frameCounters.available = 0;
                    frameCounters.count = 0;
                    frameCounters.skipped = true;
                }
                interpolatedCondition.notify_all();
            }
        }
