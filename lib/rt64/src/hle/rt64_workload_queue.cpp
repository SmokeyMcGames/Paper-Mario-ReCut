...
#       ifdef RT64_PAPER_MARIO_TRANSFORM_MATCHING
                    if (generateInterpolatedFrames) {
                        int32_t matchingTargetPairs = 0;
                        int32_t lastDrawnPair = -1;
                        for (int32_t f = 0; f < workload.fbPairCount; f++) {
                            const FramebufferPair &fbPair = workload.fbPairs[f];
                            if (fbPair.drawColorRect.isEmpty()) {
                                continue;
                            }
                            ...
                            lastDrawnPair = f;
                            if ((fbPair.colorImage.address == interpolationTargetKey.address) &&
                                (fbPair.colorImage.siz == interpolationTargetKey.siz) &&
                                (fbPair.colorImage.width == interpolationTargetKey.width)) {
                                matchingTargetPairs++;
                            }
                        }

                        if ((matchingTargetPairs != 1) || (interpolationTargetFbPairIndex != lastDrawnPair)) {
                            interpolationTargetKey = RenderTargetKey();
                            interpolationTargetFbPairIndex = -1;
